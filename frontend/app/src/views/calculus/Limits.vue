...
const conclusion = computed(() => {
  ...
  const leftVals = HS.slice().reverse().map((h) => safe(fn, c.value - h))
  const rightVals = HS.map((h) => safe(fn, c.value + h))
  const lastL = leftVals[leftVals.length - 1]
  const lastR = rightVals[rightVals.length - 1]
